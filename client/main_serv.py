...
class MainServerConn():
    def __init__(self, config, server_port = 5000):
        ...
        if not self.conn_estd:
            return
        self.send_init_to_serv(config) # send init to server
        self.HB_timer = threading.Timer(10.0, self.send_HB) # a timer to send heartbeat packets
        self.HB_timer.start()
    ...
    def send_init_to_serv(self, config):
        ...
        param config : config dictionary with information about client. Keys are CLIENTID, FILE_VECTOR, MYPORT
        '''
        self.main_serv.sendall(bytes("INIT:"+str(config['CLIENTID'])+":"+str(config['FILE_VECTOR'])+":"+str(config['MYPORT']), encoding ='utf-8')) # send info to server
        retries = constants.CLIENT_MAIN_SERV_RETRIES #how many retries to receive back success message
        conn_estd = False
        while(retries):
            try:
                self.main_serv.settimeout(constants.CLIENT_MAIN_SERV_TIMEOUT)
                success = self.main_serv.recv(4096)
                if success.decode('utf-8') == "Success!": #client init was a success
                    conn_estd = True
                    break 
                elif success.decode('utf-8') == 'HB-': # server asked to close connection, duplicate client
                    conn_estd = False
                    logger.error("Received server close from main server, init fail. (Duplicate client)")
                    break
                elif success.decode('utf-8') == "ERR:MALFORM": #malformed request received by server
                    conn_estd = False
            ...
            except socket.timeout as e: # socket time out
                retries-=1
        if (conn_estd):
    ...
    def request_file(self, file_no):
        ...
        with self.wait_HB: #make sure we are not waiting for a HB reply
            ...
            self.main_serv.sendall(bytes(f"FILE:{file_no}", encoding = "utf-8")) # send request to server
            ...
            while retries>=0:
                try:
                    ...
                    try:
                        ...
                        if success == 'DONE':
                            logger.info("Success message acknowledged by main server")
                            self.HB_timer = threading.Timer(10.0, self.send_HB)
                            self.HB_timer.start()
                            return 0
                        else:
                            logger.error("Unknown message received from main server")
        ...
        while retries>=0:
            try:
                ...
                if len(HB_resp) == 0:
                    ...
                    logger.error("Unknown message received from server. Exiting")
                    self.set_conn_status(False)
                    return
            except socket.timeout:
