...
def create_dirs(path_hier):
    ...
    logger.info("Created directories if they didn't exist at {0}"
                .format(path_hier))
    return parent
...
def client_config_generator(files_location):
    ...
    for file_idx in range(100):
        ...
        client_dict['MYPORT'] = random.choice([n for n in range(1024, 49151)
                                               if n != 5000])
        client_dict['FILE_VECTOR'] = ''.join([str(random.choice([0, 1]))
                                              for x in range(50)])

        if client_dict['FILE_VECTOR'] == "0"*50:
            logger.debug("FILE_VECTOR for name was all 0s,")
        ...
        with new_file.open('w') as f:
            ...
            logger.info("Client created with CLIENTID: "
                        f"{client_dict['CLIENTID']}")
